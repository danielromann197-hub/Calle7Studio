document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderUsersTable(users) {
        ...
        users.forEach(u => {
            ...
            const userEmail = u.email || 'Sin email';
            const userRole = u.role || 'empleado';
            
            tbody.innerHTML += `
                <tr>
                    <td><strong>${userName}</strong></td>
                    <td>${userEmail}</td>
                    <td><span class="badge ${userRole}">${userRole.toUpperCase()}</span></td>
                    <td>
                        <button class="btn-icon" title="Editar" onclick="openEditUserModal('${u.id}')"><i class="fa-solid fa-pen"></i></button>
                        <button class="btn-icon danger" title="Eliminar" onclick="deleteUser('${u.id}')"><i class="fa-solid fa-trash"></i></button>
                    </td>
                </tr>
            `;
        });
    ...
    async function createNewUser() {
        ...
        if (window.MOCK_MODE) {
            ...
            try {
                // Para crear usuarios sin cerrar la sesión del admin, usamos una instancia secundaria
                // firebaseConfig debe estar disponible globalmente desde firebase-config.js
                const secondaryApp = firebase.initializeApp(firebaseConfig, "Secondary");
                const result = await secondaryApp.auth().createUserWithEmailAndPassword(email, password);
                
                // Guardar perfil en Firestore usando la instancia principal (db)
                ...
                // Cerrar sesión en la instancia secundaria y borrarla
    ...
    window.deleteUser = async function(id) {
        if(!confirm("¿Estás seguro de que deseas eliminar este usuario? Esto borrará su perfil de la base de datos.")) return;
    ...
    window.openEditUserModal = async function(id) {
        let user = null;
        if (window.MOCK_MODE) {
            user = getMockData().users.find(u => u.id === id);
        } else {
            try {
        ...
        if(user) {
            ...
            document.getElementById('editUName').value = user.name || '';
            document.getElementById('editURole').value = user.role || 'empleado';
    ...
    async function updateUser() {
        ...
        const name = document.getElementById('editUName').value;
    ...
    function renderClients(clients) {
        ...
        clients.forEach(c => {
            ...
            if(c.status === 'lead') leadCount++;

            tbody.innerHTML += `
                <tr>
                    <td><strong>${c.name}</strong></td>
                    <td>
                        <small>${c.contact}</small><br>
                        <span style="color:var(--text-muted); font-size:0.8rem;">${c.email}</span>
                    </td>
                    <td><span class="badge ${c.status}">${c.status.toUpperCase()}</span></td>
                    <td>
                        <button class="btn-icon" title="Editar" onclick="alert('Edición en desarrollo')"><i class="fa-solid fa-pen"></i></button>
                        <button class="btn-icon danger" title="Eliminar" onclick="deleteClient('${c.id}')"><i class="fa-solid fa-trash"></i></button>
                    </td>
                </tr>
            `;
        });

        document.getElementById('clientCountTotal').textContent = clients.length;
        document.getElementById('clientCountActive').textContent = activeCount;
        document.getElementById('clientCountLeads').textContent = leadCount;
    }

    function createNewClient() {
        const name = document.getElementById('clName').value;
        const contact = document.getElementById('clContact').value;
        const email = document.getElementById('clEmail').value;
        const phone = document.getElementById('clPhone').value;
        const status = document.getElementById('clStatus').value;

        const newClient = { name, contact, email, phone, status, createdAt: new Date().toISOString() };

        if (window.MOCK_MODE) {
            const data = getMockData();
            if(!data.clients) data.clients = [];
            newClient.id = 'cl'+Date.now();
